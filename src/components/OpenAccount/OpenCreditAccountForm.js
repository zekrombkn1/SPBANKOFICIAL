...
import './OpenCreditAccountForm.css';

const OpenCreditAccountForm = () => {
    ...
    useEffect(() => {
        const checkAccountExists = async () => {
            try {
                if (userInfo) {
                    const exists = await accountExists(userInfo.userID, 'credit');
                    if (exists) {
                        setError('You already have a Credit Account.');
                    }
                }
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (userInfo) {
                ...
                await createAccount(userInfo.userID, 'credit', 20000); // Create credit account with initial balance
                navigate('/dashboard');
            }
        } catch (error) {
                ...
                <div className="col-md-6">
                    <div className="card shadow-lg p-3 mb-5 bg-white rounded">
                        <div className="card-header text-center mb-4">
                            <h2 className="card-title styled-title">Open Credit Account</h2>
                        </div>
...
export default OpenCreditAccountForm;
